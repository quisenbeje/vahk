...
; set modes
map("v",":set_mode(N)",C)
map("<C-w>",":set_mode(C)",P)
map("<C-w>",":set_mode(C)",N)
...
loop
{
	...
	;msgbox % "keystream:[" . keystream . "] ndx:[" . ndx "] out:[" . maps[ndx] . "]"
	if (maps[ndx] != "")
	{
		;msgbox "found match"
		;if (writable)
		...
		if (m_chk & md)
		{
			...
			for index, key_map in maps
			{
				...
				if (regexmatch(index,"^" . ndx) || regexmatch(ndx,"^" . index))
					errmsg .= "`n   ascii keys:[" . index . "] mapped to:[" . key_map . "]"
			}
			if (errmsg != "")
			{
